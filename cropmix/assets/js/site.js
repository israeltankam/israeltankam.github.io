(() => {
  const STATIC_DOC_VERSION = '0.2.1';
  const currentVersion = String(window.CROPMIX_VERSION || STATIC_DOC_VERSION);
  const applyVersion = value => typeof value === 'string' ? value.split(STATIC_DOC_VERSION).join(currentVersion) : value;
  if (currentVersion !== STATIC_DOC_VERSION) {
    const walker = document.createTreeWalker(document.documentElement, NodeFilter.SHOW_TEXT);
    const nodes = [];
    while (walker.nextNode()) nodes.push(walker.currentNode);
    nodes.forEach(node => { node.nodeValue = applyVersion(node.nodeValue); });
    document.querySelectorAll('[content]').forEach(el => { el.setAttribute('content', applyVersion(el.getAttribute('content'))); });
    document.querySelectorAll('[data-copy-text]').forEach(el => { el.dataset.copyText = applyVersion(el.dataset.copyText); });
  }
  ...
  if(input && results){
    let idx=[]; fetch('search-index.json').then(r=>r.json()).then(x=>idx=x.map(d=>Object.fromEntries(Object.entries(d).map(([k,v])=>[k,applyVersion(v)])))).catch(()=>{});
